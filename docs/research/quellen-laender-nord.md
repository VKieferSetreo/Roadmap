...
## 3.4 WFS/WMS Baustellen Hauptverkehrs- und Bundesfernstraßen Hamburg

- **quelle:** Baustellen auf Hauptverkehrs- und Bundesfernstraßen Hamburg
- **betreiber:** FHH, Landesbetrieb Straßen, Brücken und Gewässer (LSBG)
- **datentyp:** Baustellen auf Haupt-/Bundesfernstraßen (Bauweiser-Plattform)
- **strassentyp:** A/B + Hauptverkehrsstraßen HH
- **format:** WFS (GML), WMS
- **apiEndpunkt (verifiziert):**
  - WMS-Preview: `https://geodienste.hamburg.de/HH_WMS_Verkehr_opendata`
  - WFS „Verkehrsdaten (OpenData)" / Baustellen-Layer (Archiv-Snapshot bestätigt): `https://archiv.transparenz.hamburg.de/hmbtgarchive/HMDK/hh_wfs_verkehr_opendata_26235_snap_5.XML` (GML); GetCapabilities-Snapshot: `…hh_wfs_verkehr_opendata_26037_snap_9.XML`
  - Live-WFS-Endpunkt = Verkehr-OpenData-Dienst (siehe 3.3) → exakter Layer zu-bestätigen
- **update:** wöchentlich
- **auth:** keine
- **kosten:** keine
- **lizenz:** dl-de/by-2.0 — „FHH, Landesbetrieb Straßen, Brücken und Gewässer"
- **abdeckung:** Hamburg (Hauptnetz)
- **zugang:** offen
- **verifiziert:** ja (Datensatz + Archiv-Resources) / exakter Live-Layer zu-bestätigen
- **url:** `https://suche.transparenz.hamburg.de/dataset/baustellen-auf-hauptverkehrs-und-bundesfernstrassen-hamburg5`
- **prio:** P1
...
## 3.6 WFS Straßen- und Wegenetz Hamburg (HH-SIB)

- **quelle:** WFS Straßen- und Wegenetz Hamburg (HH-SIB, Knoten-Kanten-Modell)
- **betreiber:** FHH, BWVI; LGV
- **datentyp:** Straßen-/Wegenetz mit verschiedenen Merkmalen (Klassifizierung etc.)
- **strassentyp:** Alle (HH)
- **format:** WFS 1.1.0/2.0.0
- **apiEndpunkt (verifiziert):**
  - WFS GetCapabilities: `https://geodienste.hamburg.de/HH_WFS_Strassen_und_Wegenetz?SERVICE=WFS&REQUEST=GetCapabilities`
- **update:** irregulär
- **auth:** keine
- **kosten:** keine
- **lizenz:** dl-de/by-2.0
- **abdeckung:** Hamburg
- **zugang:** offen
- **verifiziert:** ja (Endpunkt aus offizieller Metadaten-Doku)
- **url:** GDI-DE: `https://gdk.gdi-de.org/geonetwork/srv/api/records/6D0C9998-D7A7-43A8-910E-1C61D7079A3D`
- **prio:** P2
- **sonstiges:** Attribut-Schema (Breite/Restriktionen?) per `DescribeFeatureType` prüfen — Metadaten nennen nur „verschiedene Merkmale".

## 3.7 Transparenzportal Hamburg (Dachkatalog)

- **quelle:** Transparenzportal Hamburg — Gruppe „Transport und Verkehr"
- **betreiber:** FHH (Staatsarchiv betreibt Portal)
- **datentyp:** ~100+ Geodatensätze Transport/Verkehr (Baustellen, GST-Routen, Brücken, Verkehrslage, Verkehrsstärken, automatisiertes Fahren, Verwaltungsgrenzen u.v.m.)
- **strassentyp:** Alle
- **format:** WFS, WMS, GML, CSV, GeoJSON (Filter `res_format=wfs|wms|gml`)
- **apiEndpunkt (verifiziert):**
  - Verkehr-WFS-Liste: `https://suche.transparenz.hamburg.de/dataset?groups=transport-und-verkehr&res_format=wfs`
  - Verkehr-WMS-Liste: `https://suche.transparenz.hamburg.de/dataset?groups=transport-und-verkehr&res_format=wms`
  - CKAN-artige Suche/Facetten verfügbar
- **update:** laufend
- **auth:** keine
- **kosten:** keine
- **lizenz:** je Datensatz (überwiegend dl-de/by-2.0)
- **abdeckung:** Hamburg
- **zugang:** offen
- **verifiziert:** ja
- **url:** `https://suche.transparenz.hamburg.de/`
- **prio:** P1 (Katalog)
- **sonstiges:** **Zentraler Einstieg für HH.** Weitere relevante WFS dort: `HH_WFS_Verkehrsstaerken` (Verkehrsstärken/Zählungen), `HH_WFS_automatisiertes_Fahren` (u.a. Abbiegebeziehungen). Verlinkt MetaVer (`metaver.de`) als Metadatenkatalog.
