// Eigenes Konto: Self-Service-Passwortänderung.
//
// NUR für externe Kunden-Accounts (Login über setreo-auth-extern). Interne Setreo-
// Konten (Hub/setreo-auth) verwalten ihr Passwort dort, nicht hier.
//
// Sicherheit: die E-Mail kommt aus req.ctx (vom Proxy gestempelt, nicht fälschbar) —
// der Client kann NUR sein EIGENES Passwort setzen. Das neue Passwort wird über den
// vorhandenen Provision-Upsert (PUT /internal/users, Provision-Secret server-seitig)
// in setreo-auth-extern neu gesetzt. Kein altes Passwort nötig: die Session ist vom
// Gateway bereits authentifiziert (forward_auth), die Identität steht fest.
...
export function accountRouter({ fetchImpl = globalThis.fetch, authExtern = null }) {
  const r = Router()
